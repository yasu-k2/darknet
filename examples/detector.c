...
void text_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        image im = load_image_color(input,0,0);
        //image im_t = make_image(im.w,im.h,3);
        image im_t = threshold_image(im, -1);
        image sized = letterbox_image(im, net.w, net.h);
        //image sized_t = letterbox_image(im_t, net.w, net.h);
        ...
        if (nms) do_nms_obj(boxes, probs, l.w*l.h*l.n, l.classes, nms);
        //else if (nms) do_nms_sort(boxes, probs, l.w*l.h*l.n, l.classes, nms);
        draw_detections_text(im_t, l.w*l.h*l.n, thresh, boxes, probs, masks, names, alphabet, l.classes);
        draw_detections(im, l.w*l.h*l.n, thresh, boxes, probs, masks, names, alphabet, l.classes);
        if(outfile){
            save_image(im_t, outfile);
        }
        else{
            save_image(im_t, "predictions/predictions_text");
            save_image(im, "predictions/predictions");
#ifdef OPENCV
            cvNamedWindow("predictions_text", CV_WINDOW_NORMAL);
            cvNamedWindow("predictions", CV_WINDOW_NORMAL);
            if(fullscreen){
                cvSetWindowProperty("predictions_text", CV_WND_PROP_FULLSCREEN, CV_WINDOW_FULLSCREEN);
                cvSetWindowProperty("predictions", CV_WND_PROP_FULLSCREEN, CV_WINDOW_FULLSCREEN);
            }
            show_image(im_t, "predictions_text");
            show_image(im, "predictions");
            cvWaitKey(0);
            cvDestroyAllWindows();
#endif
...
void run_detector(int argc, char **argv)
{
    ...
    if(argc < 4){
        fprintf(stderr, "usage: %s %s [train/test/valid/text] [cfg] [weights (optional)]\n", argv[0], argv[1]);
        return;
    }
    ...
    else if(0==strcmp(argv[2], "demo_text")) {
        list *options = read_data_cfg(datacfg);
        int classes = option_find_int(options, "classes", 20);
        char *name_list = option_find_str(options, "names", "data/names.list");
        char **names = get_labels(name_list);
        demo_text(cfg, weights, thresh, cam_index, filename, names, classes, frame_skip, prefix, avg, hier_thresh, width, height, fps, fullscreen);
    }
}
